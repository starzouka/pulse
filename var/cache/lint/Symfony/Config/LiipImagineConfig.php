...
class LiipImagineConfig implements \Symfony\Component\Config\Builder\ConfigBuilderInterface
{
     ...
     * @return $this
     */
    public function cacheBasePath($value): static
    {
    ...
    */
    ...
    /**
     * Enables integration with symfony/messenger if set true. Warmup image caches in background by sending messages to MQ.
    ...
    */
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['resolvers'])) {
            $output['resolvers'] = array_map(fn ($v) => $v->toArray(), $this->resolvers);
        }
        ...
        if (isset($this->_usedProperties['filterSets'])) {
            $output['filter_sets'] = array_map(fn ($v) => $v->toArray(), $this->filterSets);
        }

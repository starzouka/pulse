...
/**
 * Integration test for EmbeddableMutabilityAnalyzer.
 *
 * Tests the analyzer's ability to detect issues with entity metadata.
 */
final class EmbeddableMutabilityAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $embeddableMutabilityAnalyzer = $this->createAnalyzer();
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $embeddableMutabilityAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $embeddableMutabilityAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): EmbeddableMutabilityAnalyzer
    {
        $entityManager = $this->entityManager;
        $issueFactory = new IssueFactory();
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new EmbeddableMutabilityAnalyzer($entityManager, $issueFactory, $suggestionFactory);
    }

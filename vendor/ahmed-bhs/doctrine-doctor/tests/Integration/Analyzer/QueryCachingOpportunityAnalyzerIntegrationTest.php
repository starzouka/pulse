...
/**
 * Integration test for QueryCachingOpportunityAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class QueryCachingOpportunityAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $queryCachingOpportunityAnalyzer = $this->createAnalyzer();
        $issueCollection = $queryCachingOpportunityAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $queryCachingOpportunityAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $queryCachingOpportunityAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): QueryCachingOpportunityAnalyzer
    {
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new QueryCachingOpportunityAnalyzer($suggestionFactory);
    }

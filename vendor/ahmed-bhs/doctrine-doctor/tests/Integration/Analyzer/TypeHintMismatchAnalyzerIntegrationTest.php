...
/**
 * Integration test for TypeHintMismatchAnalyzer.
 *
 * Tests the analyzer's ability to detect issues with entity metadata.
 */
final class TypeHintMismatchAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $typeHintMismatchAnalyzer = $this->createAnalyzer();
        $issueCollection = $typeHintMismatchAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $typeHintMismatchAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $typeHintMismatchAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): TypeHintMismatchAnalyzer
    {
        $entityManager = $this->entityManager;
        $issueFactory = new IssueFactory();
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new TypeHintMismatchAnalyzer($entityManager, $issueFactory, $suggestionFactory);
    }

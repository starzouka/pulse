...
/**
 * Integration test for DTOHydrationAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class DTOHydrationAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $dtoHydrationAnalyzer = $this->createAnalyzer();
        $issueCollection = $dtoHydrationAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $dtoHydrationAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $dtoHydrationAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): DTOHydrationAnalyzer
    {
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new DTOHydrationAnalyzer($suggestionFactory);
    }

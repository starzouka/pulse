...
/**
 * Integration test for OnDeleteCascadeMismatchAnalyzer.
 *
 * Tests the analyzer's ability to detect issues with entity metadata.
 */
final class OnDeleteCascadeMismatchAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $onDeleteCascadeMismatchAnalyzer = $this->createAnalyzer();
        $issueCollection = $onDeleteCascadeMismatchAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $onDeleteCascadeMismatchAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $onDeleteCascadeMismatchAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): OnDeleteCascadeMismatchAnalyzer
    {
    ...
    private function createTwigRenderer(): TwigTemplateRenderer
    {
        $arrayLoader = new ArrayLoader([
            ...
            'on_delete_cascade_mismatch' => 'OnDelete cascade mismatch detected',
        ]);

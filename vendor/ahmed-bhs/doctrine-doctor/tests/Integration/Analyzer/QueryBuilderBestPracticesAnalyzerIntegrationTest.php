...
/**
 * Integration test for QueryBuilderBestPracticesAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class QueryBuilderBestPracticesAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $queryBuilderBestPracticesAnalyzer = $this->createAnalyzer();
        $issueCollection = $queryBuilderBestPracticesAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $queryBuilderBestPracticesAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $queryBuilderBestPracticesAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): QueryBuilderBestPracticesAnalyzer
    {
        $issueFactory = new IssueFactory();
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new QueryBuilderBestPracticesAnalyzer($issueFactory, $suggestionFactory);
    }

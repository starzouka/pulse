...
/**
 * Integration test for EmbeddableWithoutValueObjectAnalyzer.
 *
 * Tests the analyzer's ability to detect issues with entity metadata.
 */
final class EmbeddableWithoutValueObjectAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $embeddableWithoutValueObjectAnalyzer = $this->createAnalyzer();
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $embeddableWithoutValueObjectAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $embeddableWithoutValueObjectAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): EmbeddableWithoutValueObjectAnalyzer
    {
        $entityManager = $this->entityManager;
        $issueFactory = new IssueFactory();
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new EmbeddableWithoutValueObjectAnalyzer($entityManager, $issueFactory, $suggestionFactory);
    }

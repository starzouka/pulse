...
/**
 * Integration test for EntityStateConsistencyAnalyzer.
 *
 * Tests the analyzer's ability to detect issues with entity metadata.
 */
final class EntityStateConsistencyAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $entityStateConsistencyAnalyzer = $this->createAnalyzer();
        $issueCollection = $entityStateConsistencyAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $entityStateConsistencyAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $entityStateConsistencyAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): EntityStateConsistencyAnalyzer
    {
        $entityManager = $this->entityManager;
        $issueFactory = new IssueFactory();

        return new EntityStateConsistencyAnalyzer($entityManager, $issueFactory);
    }
}

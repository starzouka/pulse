...
/**
 * Integration test for InnoDBEngineAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class InnoDBEngineAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $innoDBEngineAnalyzer = $this->createAnalyzer();
        $issueCollection = $innoDBEngineAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $innoDBEngineAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $innoDBEngineAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): InnoDBEngineAnalyzer
    {
        ...
        return new InnoDBEngineAnalyzer($connection, $suggestionFactory);
    }

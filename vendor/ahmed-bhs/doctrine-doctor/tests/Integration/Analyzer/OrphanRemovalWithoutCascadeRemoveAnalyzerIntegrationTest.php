...
/**
 * Integration test for OrphanRemovalWithoutCascadeRemoveAnalyzer.
 *
 * Tests the analyzer's ability to detect issues with entity metadata.
 */
final class OrphanRemovalWithoutCascadeRemoveAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $orphanRemovalWithoutCascadeRemoveAnalyzer = $this->createAnalyzer();
        $issueCollection = $orphanRemovalWithoutCascadeRemoveAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $orphanRemovalWithoutCascadeRemoveAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $orphanRemovalWithoutCascadeRemoveAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_validates_issue_severity_is_appropriate(): void
    {
        ...
        // Ensure we have a collection instance even if empty
        self::assertInstanceOf(IssueCollection::class, $issueCollection);
    ...
    private function createAnalyzer(): OrphanRemovalWithoutCascadeRemoveAnalyzer
    {

...
/**
 * Integration test for OrderByWithoutLimitAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class OrderByWithoutLimitAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $orderByWithoutLimitAnalyzer = $this->createAnalyzer();
        $issueCollection = $orderByWithoutLimitAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $orderByWithoutLimitAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $orderByWithoutLimitAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): OrderByWithoutLimitAnalyzer
    {
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new OrderByWithoutLimitAnalyzer($suggestionFactory);
    }

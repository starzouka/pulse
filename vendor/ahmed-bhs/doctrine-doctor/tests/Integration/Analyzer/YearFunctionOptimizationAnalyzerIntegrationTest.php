...
/**
 * Integration test for YearFunctionOptimizationAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class YearFunctionOptimizationAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $yearFunctionOptimizationAnalyzer = $this->createAnalyzer();
        $issueCollection = $yearFunctionOptimizationAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $yearFunctionOptimizationAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $yearFunctionOptimizationAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): YearFunctionOptimizationAnalyzer
    {
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new YearFunctionOptimizationAnalyzer($suggestionFactory);
    }

...
/**
 * Integration test for DecimalPrecisionAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class DecimalPrecisionAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $decimalPrecisionAnalyzer = $this->createAnalyzer();
        $issueCollection = $decimalPrecisionAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $decimalPrecisionAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $decimalPrecisionAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): DecimalPrecisionAnalyzer
    {
        $entityManager = $this->entityManager;
        $issueFactory = new IssueFactory();
        $suggestionFactory = new SuggestionFactory($this->createTwigRenderer());

        return new DecimalPrecisionAnalyzer($entityManager, $issueFactory, $suggestionFactory);
    }

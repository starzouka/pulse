...
/**
 * Integration test for StrictModeAnalyzer.
 *
 * Tests the analyzer's ability to detect issues and provide actionable suggestions.
 */
final class StrictModeAnalyzerIntegrationTest extends TestCase
{
    ...
    {
        $strictModeAnalyzer = $this->createAnalyzer();
        $issueCollection = $strictModeAnalyzer->analyze(QueryDataCollection::empty());
    ...
    public function it_returns_consistent_results(): void
    {
        ...
        // Run analysis twice
        $issueCollection = $strictModeAnalyzer->analyze(QueryDataCollection::empty());
        $issues2 = $strictModeAnalyzer->analyze(QueryDataCollection::empty());
    ...
    private function createAnalyzer(): StrictModeAnalyzer
    {
        ...
        return new StrictModeAnalyzer($connection, $suggestionFactory);
    }

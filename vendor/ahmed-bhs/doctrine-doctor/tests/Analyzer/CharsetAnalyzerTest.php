...
/**
 * Test for CharsetAnalyzer.
 *
 * This analyzer detects database charset/encoding configuration issues.
 * Platform-specific detection for MySQL/MariaDB/PostgreSQL.
 *
 * Note: Full integration tests exist in CharsetAnalyzerIntegrationTest.
 * These unit tests verify basic analyzer behavior and metadata.
 */
final class CharsetAnalyzerTest extends TestCase
{
    private CharsetAnalyzer $analyzer;

    protected function setUp(): void
    {
        $connection = PlatformAnalyzerTestHelper::createTestConnection();

        $this->analyzer = new CharsetAnalyzer(
            $connection,
    ...
    public function it_returns_issue_collection(): void
    {
        ...
        // Assert: Should return an IssueCollection (may be empty for SQLite)
    ...
    {
        // Arrange: SQLite should be skipped (no charset config)
        $queries = QueryDataBuilder::create()->build();

        // Act
        $issues = $this->analyzer->analyze($queries);

        // Assert: Should return empty collection for SQLite
